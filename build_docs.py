...
"""
DOCS_BUILD_PATH = Path("docs/_build/eip712")
LATEST_PATH = DOCS_BUILD_PATH / "latest"
STABLE_PATH = DOCS_BUILD_PATH / "stable"


class EIP712DocsBuildError(Exception):
    pass
...
def build_docs(path: Path) -> Path:
    ...
    try:
        ...
        raise EIP712DocsBuildError(f"Command 'sphinx-build docs {path}' failed.") from err
...
def main():
    ...
    if event_name == "push" or is_ephemeral:
        ...
        if not tag:
            raise EIP712DocsBuildError("Unable to find release tag.")
