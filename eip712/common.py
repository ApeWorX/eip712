...
def create_permit_def(eip=2612, **header_fields):
    if eip == 2612:
        ...
        class Permit(EIP2612):
            eip712_name_: string | None = header_fields.get("name", None)
            eip712_version_: string | None = header_fields.get("version", None)
            eip712_chainId_: uint256 | None = header_fields.get("chainId", None)
            eip712_verifyingContract_: string | None = header_fields.get(
                "verifyingContract", None
            )
            eip712_salt_: bytes32 | None = header_fields.get("salt", None)
